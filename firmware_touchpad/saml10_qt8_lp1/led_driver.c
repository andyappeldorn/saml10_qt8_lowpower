...
extern uint8_t                    interrupt_cnt_led;
...
#define LED_SCROLL_PERIOD 30

led_scroll_t led_scroll_data[2];

uint8_t gesture_or_pos_mode = MODE_POS;

volatile uint8_t led_1msec_interrupt_cnt_led;
...
{
	if (led_time_track != interrupt_cnt_led) {
		if (interrupt_cnt_led > led_time_track) {
			led_1msec_interrupt_cnt_led += (interrupt_cnt_led - led_time_track);
		} else {
			led_1msec_interrupt_cnt_led += (interrupt_cnt_led + DEF_TOUCH_MEASUREMENT_PERIOD_MS - led_time_track);
		}
		led_time_track = interrupt_cnt_led;
	}
}
...
void led_blink_scroll(void)
{
	if (led_current_action == LED_SCROLL) {
		if (led_1msec_interrupt_cnt_led > LED_SCROLL_PERIOD) {
			led_1msec_interrupt_cnt_led = 0;
		...
		led_blink_time_cnt += led_1msec_interrupt_cnt_led;
		led_1msec_interrupt_cnt_led = 0;
...
{
	if (blink_cnt != 0 && led_current_action == LED_NO_ACTION) {
...
void led_scroll(uint8_t hor, uint8_t hor_dir, uint8_t ver, uint8_t ver_dir)
{
	...
	if (led_current_action == LED_NO_ACTION) {
